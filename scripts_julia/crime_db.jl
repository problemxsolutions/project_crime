...
wx_data_to_db = CSV.read(filename, DataFrame)

tabel_name = "weather_jl"
exe_s = string(
  "date       timestamp,",
  "datatype   varchar(4),",
  "station    varchar(17),",
...
temperature_data = CSV.read(filename, DataFrame)
...
moon_phase_data = CSV.read(filename, DataFrame)
...
suntimes_data = CSV.read(filename, DataFrame)
# *** Still need to create table processes ***
...
employment = CSV.read(filename, DataFrame)
# *** Still need to create table processes ***
...
doleta_data = CSV.read(filename, DataFrame)
# *** Still need to create table processes ***
...
bls_data = CSV.read(filename, DataFrame)
# *** Still need to create table processes ***
...
db_crime_data = CSV.read(filename, DataFrame)
